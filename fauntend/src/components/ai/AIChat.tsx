...
export function AIChat({ className = '' }: AIChatProps) {
    ...
    return (
        <div className={`flex flex-col h-[600px] glass-card w-full max-w-full ${className}`}>
            {/* Header */}
            <div className="p-3 sm:p-4 border-b border-[var(--border-color)] overflow-visible">
                {/* Row 1: Title, Model, Clear */}
                            ...
                            <AnimatePresence>
                                {showModelSelect && (
                                    <motion.div
                                        ...
                                        className={`absolute right-0 w-48 p-1 rounded-lg bg-[var(--bg-card)] border border-[var(--border-color)] shadow-xl z-50 ${dropdownPosition === 'bottom' ? 'top-full mt-1' : 'bottom-full mb-1'
                                            }`}
                                    >
                                        {MODEL_OPTIONS.map(opt => (
                                            <button
                                                ...
                                                className={`w-full flex flex-col items-start p-2 rounded-lg text-left transition-colors ${model === opt.value ? 'bg-purple-500/20' : 'hover:bg-[var(--bg-secondary)]'
                                                    }`}
                                            >
                ...
                {/* Row 2: Session Key/History, Rate Limit */}
                <div className="flex items-center justify-between gap-2 flex-wrap">
                    {/* Rate Limit - Gemini only */}
                    {supportsAdvancedFeatures && (
                        <span className={`text-xs px-2 py-1 rounded-full ${rateLimit.remaining > 10 ? 'bg-green-500/20 text-green-400' :
                                ...
                                <AnimatePresence>
                                    {showSessionMenu && (
                                        <motion.div
                                            ...
                                            animate={{ opacity: 1, y: 0 }}
                                            exit={{ opacity: 0, y: dropdownPosition === 'bottom' ? -10 : 10 }}
                                            className={`absolute right-0 w-72 p-2 rounded-lg bg-[var(--bg-card)] border border-[var(--border-color)] shadow-xl z-50 ${dropdownPosition === 'bottom' ? 'top-full mt-1' : 'bottom-full mb-1'
                                                }`}
                                        >
                    ...
                    messages.map(msg => (
                        ...
                        >
                            ...
                            <div className={`flex-1 min-w-0 max-w-[85%] sm:max-w-[80%] ${msg.role === 'user' ? 'text-right' : ''}`}>
                                <div className={`inline-block p-3 rounded-xl break-words ${msg.role === 'user'
                                ...
                                {/* Meta */}
                                <div className={`flex items-center gap-2 mt-1 text-xs text-[var(--text-muted)] ${msg.role === 'user' ? 'justify-end' : ''
                                    }`}>
            ...
            <div className="p-4 border-t border-[var(--border-color)]">
                ...
                <div className="flex gap-2">
                    ...
                    <button
                        ...
                        className={`p-2.5 rounded-lg transition-colors ${supportsAdvancedFeatures
                    ...
                    <button
                        ...
                        disabled={!supportsAdvancedFeatures}
                        className={`flex items-center gap-1.5 px-3 py-2.5 rounded-lg transition-colors text-xs ${!supportsAdvancedFeatures
                            ? 'bg-[var(--bg-secondary)] text-[var(--text-muted)] opacity-40 cursor-not-allowed'
                            : webSearch
